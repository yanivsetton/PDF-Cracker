...
def run_mpbf(pdf_file_path: Path):
    wordlist_gen = (x for x in permutations(ascii_lowercase + digits, 2))
    p = Pool(30)
    result = p.starmap(bruteforce_pdf, [(pdf_file_path, wordlist) for wordlist in wordlist_gen])

def bruteforce_pdf(pdf_file_path: Path, start) -> Union[str, None]:
    ...
    wordlist = ("".join(start + y) for y in permutations(ascii_lowercase + digits, 6))
